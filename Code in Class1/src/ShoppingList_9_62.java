//import java.util.ArrayList;
//
//public class ShoppingList_9_6 {
//    static ArrayList<Item> shoppingList = new ArrayList<>();
//
//    public ShoppingList_9_6(ArrayList<Item> shoppingList) {
//        this.shoppingList = shoppingList;
//    }
//
//    public ShoppingList_9_6() {
//
//    }
//
//    public void addItem(String name, int ct) {
////        Change to if item exists then change count to ct
//         int indx = nameIsThere(name);
//        if (indx  != -1)
//        {
//            shoppingList.remove(indx);
//            Item nItem = new Item( name, ct);
//            shoppingList.add(nItem);
////            Do something
//        }
//        else {
////            Do something else
//            Item item = new Item(name, ct);
//            shoppingList.add(item);
//
//        }
//    }
//
//
//    public int nameIsThere(String inName) {
//        int theItemInx = -1;
//        int ct = 0;
//        for (Item item : shoppingList) {
//            if (item.getName().equals(inName)) {
//                theItemInx = ct;
//                break;
//            }
//            ct++;
//        }
//        return theItemInx;
//    }
//
//    public static void showList() {
//        for (Item item : shoppingList) {
//            System.out.printf("\nItem:%s Ct:%s", item.getName(),
//                    item.getCt());
//        }
//    }
//
//    public int getSize() {
//        return shoppingList.size();
//
//    }
//
//    public boolean removeItem(String inItem) {
//        boolean gotIt = false;
//        int ct = 0;
//        for (Item item : shoppingList) {
//            if (item.getName().equals(inItem)) {
//                shoppingList.remove(ct);
//                gotIt = true;
//                break;
//            }
//            ct++;
//        }
//        return gotIt;
//    }
//
//    public static ArrayList<Item> getItems(boolean gotIt) {
////        Makes an array list
//        ArrayList<Item> items = new ArrayList<>();
////        Goes through the arraylist
//        for (Item item : shoppingList) {
//            if (item.getGotIt() == gotIt) {
////                if the item has it, then it prints to the arraylist
//                items.add(item);
//            }
//
//
//        }
//        // Todo: return an ArrayList of item that
//        //      match the gotIt true or false value.
//        //      For example if set to True, then return
//        //      an ArrayList of Item with .
//        return new ArrayList<Item>();
//    }
//
//    public static boolean renameItem(String origName, String newName) {
////        gotIt starts as false
//        Boolean gotIt = false;
//        String: origName = "origName";
////        If origname is found then it will find it and set gotIt to true
//        if(origName == "origName" ) {
//
//            origName = "newName";
//            gotIt = true;
//        }
//        //ToDo: Find an item name origName and change its name to newName
//        // If found then return true
//        // If not found return false
//        return gotIt;
//    }
//}
//
//class Item {
//    private String name;
//// This is my boolean variable gotIt
//    public Boolean getGotIt() {
//        return gotIt;
//    }
//
//    public void setGotIt(Boolean gotIt) {
//        this.gotIt = gotIt;
//    }
//
//    private Boolean gotIt;
//    private int ct;
//    // ToDo: Add this attribute with getters ,setters
//    //        and toString updated
//    // ToDo: Adjust the constructor of Shopping list and
//    //        make sure it works right
//    //private boolean gotIt;
//
//    public Item(String name, int ct) {
//        this.name = name;
//        this.ct = ct;
//    }
//
//    public String getName() {
//        return name;
//    }
//
//    public void setName(String name) {
//        this.name = name;
//    }
//
//    public int getCt() {
//        return ct;
//    }
//
//    public void setCt(int ct) {
//        this.ct = ct;
//    }
//
//    @Override
//    public String toString() {
//        return "Item :" +
//                "name='" + name + '\'' +
//                ", ct=" + ct;
//    }
//}
